  ...
  if (!r.dependsOn) {
    ...
    return r.dependsOn.map(d => {
      ...
      const match = allResources.find(r1 => {
        ...
        if (!keys.every(k => Object.keys(r1.resourceDefinitions).indexOf(k) >= 0)) {
          return false;
        } // Do values match?
      ...
      if (match) {
        ...
        return {
          error: `A resource (${r.resourceName}: ${JSON.stringify(resourceDefinition)}) is missing its dependency on ${JSON.stringify(r.dependsOn[0])}`
        };
      }
    });
  }
